...
flash mode and brown it out, or be overridden by the chip's own internal pull at reset. Every one of
those defeats the security goal, and some of them **fail silently without ever running the wipe**
...
destruction (never wipes) but the CORRECT PASSWORD IS STILL REQUIRED to boot**, with no gate bypass
