...
- **A recovered or cracked password does more than unlock: it can also force a wipe.** On a headless
...
- SD overwrite cannot guarantee destruction of FTL-remapped cells; encryption-at-rest is the fix.
  (The stock-SD path is also **file + free-space overwrite only, with no guaranteed format/secure-erase**;
  see SPEC §8.)
